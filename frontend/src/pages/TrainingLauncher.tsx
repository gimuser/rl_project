...
const DEFAULT_MODEL=(name:string):TrainingModelConfig=>({name,learning_rate:0.001,gamma:0.95,batch_size:2048,hidden_dim:128,max_total_updates:5_000_000,cql_alpha:1});
function modelInfo(name:string){return MODELS.find(m=>m.name===name)??MODELS[0];}
function sizeLabel(bytes:number|null|undefined){if(bytes==null)return "—"; if(bytes<1024*1024)return `${(bytes/1024).toFixed(1)} KB`; return `${(bytes/1024/1024).toFixed(1)} MB`;}
...
export function TrainingLauncher(){
  ...
  useEffect(()=>{void getDatasetLineage().then(setLineage).catch(()=>setLineage(null));},[]);
  ...
  return <div className="lux-training">
    ...
    <section className="lux-card"><div className="lux-card-head"><div><div className="lux-card-label">STEP 1 · ALGORITHM SELECTION</div><h2>{selectionLabel}</h2></div><span className="lux-pill">Hard data · no synthetic fallback</span></div>
      <div className="lux-model-selector-grid lux-model-selector-grid--four">{MODELS.map(m=>{const active=selected.includes(m.name);return <button key={m.name} type="button" className={`lux-model-choice ${active?"lux-model-choice--selected":""}`} onClick={()=>toggle(m.name)}><div className="lux-model-choice__top"><span className="lux-model-choice__tag">{m.tag}</span><span className="lux-model-choice__check">{active?"✓":""}</span></div><strong>{m.title}</strong><p>{m.detail}</p><small>{active?"Selected · configure below":"Click to include"}</small></button>;})}</div>
    </section>

    {lineage&&<section className="lux-card" style={{marginTop:18}}><div className="lux-card-head"><div><div className="lux-card-label">DATASET LINEAGE</div><h2>Exactly what this run will use</h2></div><span className="lux-pill">{lineage.synthetic_data?"Synthetic enabled":"Hard data · streaming"}</span></div><p className="lux-muted" style={{marginTop:0}}>{lineage.lineage}</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12,marginTop:16}}>{lineage.sources.map(source=><div key={source.path} style={{border:"1px solid #dbe3ee",borderRadius:14,padding:14}}><div style={{fontSize:".68rem",fontWeight:900,letterSpacing:".08em",color:"#6b7b93"}}>{source.role}</div><strong style={{display:"block",marginTop:6,fontSize:".84rem",wordBreak:"break-word"}}>{source.path}</strong><div style={{marginTop:8,color:source.exists?"#28744a":"#a33d3d",fontWeight:800}}>{source.exists?sizeLabel(source.size_bytes):"MISSING"}</div></div>)}</div><div className="lux-mini-grid" style={{marginTop:16}}><div><span>TRAIN rows</span><strong>{lineage.split.train_rows?.toLocaleString()??"—"}</strong></div><div><span>MODEL-TRAIN incidents</span><strong>{lineage.split.model_train_incidents?.toLocaleString()??"—"}</strong></div><div><span>VALIDATION incidents</span><strong>{lineage.split.validation_incidents?.toLocaleString()??"—"}</strong></div><div><span>UNSEEN TEST rows</span><strong>{lineage.split.test_rows?.toLocaleString()??"—"}</strong></div><div><span>TRAIN/TEST overlap</span><strong>{lineage.split.train_test_overlap??0}</strong></div><div><span>VAL/TEST overlap</span><strong>{lineage.split.validation_test_overlap??0}</strong></div></div></section>}

    {configured&&!complete&&<section className="lux-card" style={{marginTop:18}}><div className="lux-card-head"><div><div className="lux-card-label">STEP {step+2} · MODEL CONFIGURATION</div><h2>{modelInfo(configured).title}</h2></div><span className="lux-pill">Model {step+1} of {selected.length}</span></div><p className="lux-muted">Only this model is shown now. Continue to unlock the next selected model.</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:22,marginTop:18}}><NumericField label="Learning rate" value={modelConfigs[configured].learning_rate} onChange={v=>updateModel(configured,"learning_rate",v)} min={0.0000001} max={1} step={0.0001}/><NumericField label="Discount factor γ" value={modelConfigs[configured].gamma} onChange={v=>updateModel(configured,"gamma",v)} min={0} max={1} step={0.01}/><NumericField label="Batch size" value={modelConfigs[configured].batch_size} onChange={v=>updateModel(configured,"batch_size",v)} min={32} max={16384} step={32} integer/><NumericField label="Hidden dimension" value={modelConfigs[configured].hidden_dim} onChange={v=>updateModel(configured,"hidden_dim",v)} min={16} max={2048} step={16} integer/>{configured==="cql"&&<NumericField label="CQL α" value={modelConfigs[configured].cql_alpha??1} onChange={v=>updateModel(configured,"cql_alpha",v)} min={0} max={100} step={0.01}/>}</div>
