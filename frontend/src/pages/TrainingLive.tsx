    ...
    <div className="legacy-line-chart" style={{ "--chart-accent": accent } as React.CSSProperties} role="img" aria-label={`${label} line chart`}>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-hidden="true">
        ...
        {coordinates.map((p, index) => (
          <circle key={`${labels[index]}-${index}`} cx={p.x} cy={p.y} r="3.5">
            <title>{`${labels[index]} · ${formatter(values[index])}`}</title>
          </circle>
        ))}
      </svg>
...
function HistoryChart({
  ...
  const formatter = (value: number) => metric === "total_updates"
    ? nf.format(Math.round(value))
    : percent ? `${(value * 100).toFixed(1)}%` : value.toFixed(4);

  return (
    <article className="legacy-chart-panel">
      <div className="legacy-chart-panel__header">
        <div><p className="eyebrow">TRAINING CURVE</p><h2>{title}</h2></div>
        <span>{visible.length} epochs</span>
      </div>
      <LegacyChart values={values} labels={labels} label={title} accent={accent} formatter={formatter} />
      {visible.length > 0 && (
        <div className="legacy-chart-panel__meta">
          <span>min {fmt(Math.min(...values), 5)}</span>
          <span>max {fmt(Math.max(...values), 5)}</span>
        </div>
      )}
    </article>
  );
...
function ProgressChart({ progress }: { progress: ProgressState }) {
  ...
  return (
    <article className="legacy-chart-panel legacy-chart-panel--progress">
      <div className="legacy-chart-panel__header">
        <div><p className="eyebrow">LIVE STREAM</p><h2>Epoch progress</h2></div>
        <span>Epoch {number(progress.epoch ?? 0)} in progress</span>
      </div>
      <div className="progress-plot">
        <div className="progress-plot__scale"><span>0%</span><strong>{percent.toFixed(1)}%</strong><span>100%</span></div>
        <div className="progress-plot__track"><i style={{ width: `${percent}%` }} /></div>
        <div className="progress-plot__axis"><span>Start</span><span>Current stream position</span><span>Complete</span></div>
      </div>
      <div className="legacy-chart-panel__meta progress-meta">
        <span>Rows {number(processed)} / {number(total)}</span>
        <span>Chunks {number(progress.chunks_processed ?? 0)} / {number(progress.chunks_total ?? 0)}</span>
        <span>Updates {number(progress.total_updates ?? 0)}</span>
      </div>
    </article>
  );
}
...
function ActionsChart({ history, windowSize }: { history: AuthoritativeHistoryPoint[]; windowSize: number }) {
  ...
  return (
    <article className="legacy-chart-panel">
      ...
      {visible.length ? (
        <div className="legacy-action-chart">
          <div className="legacy-action-chart__plot">
            {actions.map((action) => {
              ...
              const labels = visible.map((point) => `E${point.epoch}`);
              return (
                <LegacyChart key={action.key} values={values} labels={labels} label={action.name} accent={action.accent} formatter={(value) => `${(value * 100).toFixed(0)}%`} />
              );
            })}
          </div>
      ...
      ) : <div className="chart-empty">Action distribution appears after the first completed epoch.</div>}
    </article>
  );
}
...
export function TrainingLive() {
  ...
  const t = state?.results?.training as any;
  const history = (t?.history ?? []) as AuthoritativeHistoryPoint[];
  ...
  const progress = (t?.progress ?? null) as ProgressState | null;
  ...
  const progressVisible = state?.status === "running" && progress?.stage === "epoch_in_progress";
  ...
  const bestStats = useMemo(() => {
    ...
    return {
      epoch: (t?.best_epoch ?? byValidation?.epoch ?? null) as number | null,
      validation: byValidation ? pointValue(byValidation, "validation_score") : null,
      reward: byReward ? pointValue(byReward, "policy_reward") : null,
      efficiency: byEfficiency ? pointValue(byEfficiency, "reward_efficiency") : null,
    };
    ...
    <div className="live-page">
      <header className="live-hero">
        <div>
          ...
          <h1>{t?.display_name ?? t?.model_name ?? "Training monitor"}</h1>
      ...
      <section className="live-kpis">
        ...
        <div><span>Algorithm</span><strong>{t?.display_name ?? t?.algorithm ?? progress?.display_name ?? "—"}</strong></div>
        <div><span>Epoch</span><strong>{progressVisible ? `${number(progress?.epoch ?? 1)} in progress` : `${number(t?.actual_epochs ?? latest?.epoch ?? 0)} / ${number(t?.epochs ?? 0)}`}</strong></div>
        <div><span>Updates</span><strong>{number(t?.total_updates_used ?? progress?.total_updates ?? 0)}</strong></div>
        <div><span>Best epoch</span><strong>{number(bestStats.epoch)}</strong></div>
        <div><span>Stop reason</span><strong>{t?.stopping_reason ?? "learning"}</strong></div>
      </section>

      {progressVisible && <section className="realtime-progress">
        <div className="realtime-progress__head"><div><p className="eyebrow">REAL-TIME TRAINING PROGRESS</p><h2>Epoch {number(progress?.epoch ?? 1)} in progress</h2><p>The epoch is still computing. Completed-epoch curves remain separate from live streaming telemetry.</p></div><strong>{Number(progress?.progress_percent ?? 0).toFixed(1)}%</strong></div>
        <div className="realtime-progress__bar"><i style={{ width: `${Math.max(0, Math.min(100, Number(progress?.progress_percent ?? 0)))}%` }} /></div>
        <div className="realtime-progress__stats"><div><span>Rows processed</span><strong>{number(progress?.source_rows_processed ?? 0)} / {number(progress?.source_rows_total ?? 0)}</strong></div><div><span>Chunks</span><strong>{number(progress?.chunks_processed ?? 0)} / {number(progress?.chunks_total ?? 0)}</strong></div><div><span>Optimizer updates</span><strong>{number(progress?.total_updates ?? 0)}</strong></div></div>
      </section>}

      <section className="live-grid live-grid--four">
        <div className="metric"><span>Policy reward</span><strong>{fmt(t?.policy_reward ?? latest?.policy_reward, 6)}</strong></div>
        <div className="metric"><span>Validation score</span><strong>{fmt(t?.validation_score ?? latest?.validation_score, 4)}</strong></div>
        <div className="metric"><span>Reward efficiency</span><strong>{fmt(t?.reward_efficiency ?? latest?.reward_efficiency, 4)}</strong></div>
        <div className="metric"><span>Live cycle</span><strong>{liveTotal ? `${number(liveTotal)} actions` : "waiting"}</strong></div>
      </section>

      <section className="best-grid">
        <article className="best-card"><span>BEST EPOCH</span><strong>{number(bestStats.epoch)}</strong><small>Validation-selected checkpoint</small></article>
        <article className="best-card"><span>BEST VALIDATION</span><strong>{bestStats.validation == null ? "—" : `${(bestStats.validation * 100).toFixed(1)}%`}</strong><small>Highest persisted validation score</small></article>
        <article className="best-card"><span>BEST POLICY REWARD</span><strong>{bestStats.reward == null ? "—" : bestStats.reward.toFixed(4)}</strong><small>Highest persisted policy reward</small></article>
        <article className="best-card"><span>BEST EFFICIENCY</span><strong>{bestStats.efficiency == null ? "—" : `${(bestStats.efficiency * 100).toFixed(1)}%`}</strong><small>Highest persisted reward efficiency</small></article>
      </section>

      <section className="chart-toolbar">
        <div><strong>Telemetry window</strong><span>{history.length} persisted epochs</span></div>
        <label>Window <input type="range" min="10" max={maxWindow} value={effectiveWindow} onChange={(event) => { setFollow(false); setWindowSize(Number(event.target.value)); }} /><b>{effectiveWindow}</b></label>
        <button className={follow ? "toggle active" : "toggle"} onClick={() => setFollow((value) => !value)}>{follow ? "Following latest" : "Manual window"}</button>
      </section>

      <section className="charts-grid">
        {progressVisible && <ProgressChart progress={progress ?? {}} />}
        <HistoryChart title="Training loss" history={history} metric="loss" windowSize={effectiveWindow} accent="#2563eb" />
        <HistoryChart title="Policy reward" history={history} metric="policy_reward" windowSize={effectiveWindow} accent="#12805c" />
        <HistoryChart title="Validation score" history={history} metric="validation_score" windowSize={effectiveWindow} accent="#7c3aed" percent />
        <HistoryChart title="Reward efficiency" history={history} metric="reward_efficiency" windowSize={effectiveWindow} accent="#a7680a" percent />
        <HistoryChart title="Cumulative optimizer updates" history={history} metric="total_updates" windowSize={effectiveWindow} accent="#c43d4b" />
        <ActionsChart history={history} windowSize={effectiveWindow} />
      </section>

      <section className="live-bottom-grid">
        <article>
          <div className="panel-title">Selected models</div>
          <div className="chips">{selected.length ? selected.map((name: string) => <span key={name}>{name}</span>) : <span>—</span>}</div>
          <div className="panel-title">Champion</div>
          <strong>{best?.display_name ?? best?.name ?? "Not selected until comparison completes"}</strong>
        </article>
        <article>
          <div className="panel-title">80-alert live holdout</div>
          <div className="live-stats"><span>Expected<strong>{LIVE_EXPECTED}</strong></span><span>Considered<strong>{live?.alerts_considered ?? "—"}</strong></span><span>Processed<strong>{live?.alerts_processed ?? "—"}</strong></span></div>
          <p>Human review <strong>{live?.human_review_routed ?? "—"}</strong></p>
          ...
          {liveTotal > 0 && <p>Actions: {Object.entries(liveDistribution).map(([action, count]) => `${action} ${count}`).join(" · ")}</p>}
        </article>
      </section>
    </div>
  );
