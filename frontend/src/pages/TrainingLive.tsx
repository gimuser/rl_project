...
function LegacyChart({
  ...
  return (
    <div className="legacy-line-chart" style={{ "--chart-accent": accent } as any} role="img" aria-label={`${label} line chart`}>
...
function HistoryChart({
  ...
  const formatter = (value: number) =>
    metric === "total_updates" ? nf.format(Math.round(value)) : percent ? `${(value * 100).toFixed(1)}%` : value.toFixed(4);
  return (
...
function ActionsChart({ history, windowSize }: { history: AuthoritativeHistoryPoint[]; windowSize: number }) {
  ...
  return (
    <article className="legacy-chart-panel">
      <div className="legacy-chart-panel__header">
        <div><p className="eyebrow">FINAL POLICY</p><h2>Action distribution</h2></div>
        <span>Dynamic by epoch</span>
      </div>
      {visible.length ? (
...
export function TrainingLive() {
  ...
  // Keep the most recent persisted run visible after completion/stopping.
  const t = (state?.results?.training as any) ?? null;
  const history = (t?.history ?? []) as AuthoritativeHistoryPoint[];
  const latest = history.at(-1);
  const comparison = (state?.results?.comparison as any) ?? null;
  const live = (state?.results?.live_inference as any) ?? null;
  const best = comparison?.best as any;
  const selected = t?.selected_models ?? [];
  const progress = (t?.progress ?? null) as ProgressState | null;
  const displayRun = Boolean(t && history.length);
  const historicalRun = displayRun && !activeRun;
  ...
  const bestStats = useMemo(() => {
    ...
    const byValidation = [...history]
      .filter((p) => Number.isFinite(Number((p as any).validation_score)))
      .sort((a, b) => Number((b as any).validation_score ?? -Infinity) - Number((a as any).validation_score ?? -Infinity))[0];
    ...
    <div className="live-page">
      <header className="live-hero">
        <div>
          ...
          <h1>{displayRun ? (t?.display_name ?? t?.model_name ?? "Training monitor") : "Training monitor"}</h1>
        ...
        <div className="live-actions">
          ...
          <button className="live-btn" onClick={() => navigate("/training/history")}>Training history</button>
          {activeRun && <button className="live-btn live-btn--danger" disabled={stopping} onClick={() => void stop()}>{stopping ? "Stopping…" : "Stop training"}</button>}
        </div>
      </header>
      ...
      <section className="live-kpis">
        ...
        <div><span>Algorithm</span><strong>{displayRun ? (t?.display_name ?? t?.algorithm ?? "—") : "—"}</strong></div>
        <div><span>Epoch</span><strong>{progressVisible ? `${number(progress?.epoch ?? 1)} in progress` : displayRun ? `${number(t?.actual_epochs ?? latest?.epoch ?? 0)} / ${number(t?.epochs ?? 0)}` : "—"}</strong></div>
        <div><span>Updates</span><strong>{displayRun ? number(t?.total_updates_used ?? latest?.total_updates ?? 0) : "—"}</strong></div>
        <div><span>Best epoch</span><strong>{displayRun ? number(bestStats.epoch) : "—"}</strong></div>
        <div><span>Stop reason</span><strong>{displayRun ? (t?.stopping_reason ?? (activeRun ? "learning" : "completed")) : "—"}</strong></div>
      </section>
      ...
      <section className="live-grid live-grid--four"><div className="metric"><span>Policy reward</span><strong>{displayRun ? fmt(t?.policy_reward ?? latest?.policy_reward, 6) : "—"}</strong></div><div className="metric"><span>Validation score</span><strong>{displayRun ? fmt(t?.validation_score ?? latest?.validation_score, 4) : "—"}</strong></div><div className="metric"><span>Reward efficiency</span><strong>{displayRun ? fmt(t?.reward_efficiency ?? latest?.reward_efficiency, 4) : "—"}</strong></div><div className="metric"><span>Live cycle</span><strong>{activeRun && liveTotal ? `${number(liveTotal)} actions` : historicalRun ? "training finished" : "waiting"}</strong></div></section>

      <section className="best-grid"><article className="best-card"><span>BEST EPOCH</span><strong>{displayRun ? number(bestStats.epoch) : "—"}</strong><small>Validation-selected checkpoint</small></article><article className="best-card"><span>BEST VALIDATION</span><strong>{displayRun && bestStats.validation != null ? `${(bestStats.validation * 100).toFixed(1)}%` : "—"}</strong><small>Highest persisted validation score</small></article><article className="best-card"><span>BEST POLICY REWARD</span><strong>{displayRun && bestStats.reward != null ? bestStats.reward.toFixed(4) : "—"}</strong><small>Highest persisted policy reward</small></article><article className="best-card"><span>BEST EFFICIENCY</span><strong>{displayRun && bestStats.efficiency != null ? `${(bestStats.efficiency * 100).toFixed(1)}%` : "—"}</strong><small>Highest persisted reward efficiency</small></article></section>
      ...
      <section className="live-bottom-grid"><article><div className="panel-title">Selected models</div><div className="chips">{selected.length ? selected.map((name: string) => <span key={name}>{name}</span>) : <span>—</span>}</div><div className="panel-title">Champion</div><strong>{displayRun ? (best?.display_name ?? best?.name ?? "Not selected until comparison completes") : "—"}</strong></article><article><div className="panel-title">80-alert live holdout</div><div className="live-stats"><span>Expected<strong>{LIVE_EXPECTED}</strong></span><span>Considered<strong>{displayRun ? (live?.alerts_considered ?? "—") : "—"}</strong></span><span>Processed<strong>{displayRun ? (live?.alerts_processed ?? "—") : "—"}</strong></span></div><p>Human review <strong>{displayRun ? (live?.human_review_routed ?? "—") : "—"}</strong></p><p>{liveComplete ? "All 80 real live alerts were processed." : activeRun ? "The live holdout is complete only when all 80 alerts have been considered and processed." : historicalRun ? "Training finished. Persisted run telemetry remains available here." : "The live holdout is waiting for a promoted model."}</p>{liveTotal > 0 && <p>Actions: {Object.entries(liveDistribution).map(([action, count]) => `${action} ${count}`).join(" · ")}</p>}</article></section>
    </div>
  );
}
