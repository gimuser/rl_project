...
function Chart({ title, history, field, color, windowSize, percent = false }: {
  title: string; history: AuthoritativeHistoryPoint[]; field: Key; color: string; windowSize: number; percent?: boolean;
}) {
  ...
  const min = min0 - pad, max = max0 + pad, span = max - min || 1;
  ...
  const handleMove = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    let nearest = pts[0];
    for (const point of pts) if (Math.abs(point.x - svgX) < Math.abs(nearest.x - svgX)) nearest = point;
    setHovered({ epoch: nearest.e, value: nearest.v, x: nearest.x, y: nearest.y });
  };

  const displayValue = (value: number) => percent ? `${(value * 100).toFixed(2)}%` : field === "total_updates" ? nf.format(Math.round(value)) : value.toFixed(6);
  ...
  return <div className="chart-card">
    ...
    <div className="telemetry-chart-wrap">
      <svg viewBox={`0 0 ${width} ${height}`} className="telemetry-svg" role="img" aria-label={title} onMouseMove={handleMove} onMouseLeave={() => setHovered(null)}>
        {[0,.25,.5,.75,1].map(r => {
          const y = top + r * ph, val = max - r * span;
      ...
      {hovered && <div className="chart-tooltip" style={{ left: `${Math.min(Math.max((hovered.x/width)*100,12),86)}%`, top: `${Math.max((hovered.y/height)*100-18,8)}%` }}><strong>Epoch {hovered.epoch}</strong><span>{title}: {displayValue(hovered.value)}</span></div>}
    </div>
...
function ActionsChart({ history, windowSize }: { history: AuthoritativeHistoryPoint[]; windowSize: number }) {
  ...
  const series=Object.keys(colors).map(action=>({action,points:visible.map((p,i)=>{
    const c=Number((p.action_distribution as any)?.[action]??0); const total=Object.values(p.action_distribution??{}).reduce<number>((s,v)=>s+Number(v||0),0)||1;
    return {x:left+i/Math.max(1,visible.length-1)*pw,y:top+(1-c/total)*ph,e:p.epoch};
  })}));

  const handleMove=(event:React.MouseEvent<SVGSVGElement>)=>{
    const rect=event.currentTarget.getBoundingClientRect(); const svgX=((event.clientX-rect.left)/rect.width)*width;
    let index=0,bestDistance=Infinity;
    visible.forEach((p,i)=>{const x=left+i/Math.max(1,visible.length-1)*pw;const d=Math.abs(x-svgX);if(d<bestDistance){bestDistance=d;index=i;}});
    const point=visible[index], values=(point.action_distribution??{}) as Record<string,number>;
    const total=Object.values(values).reduce((s,v)=>s+Number(v||0),0)||1;
    setHovered({epoch:point.epoch,x:left+index/Math.max(1,visible.length-1)*pw,values:{allow:Number(values.allow||0)/total,block:Number(values.block||0)/total,human_review:Number(values.human_review||0)/total}});
  };
  ...
  return <div className="chart-card">
    ...
    <div className="telemetry-chart-wrap">
      ...
      {hovered && <div className="chart-tooltip chart-tooltip--multi" style={{left:`${Math.min(Math.max((hovered.x/width)*100,16),80)}%`,top:"10%"}}><strong>Epoch {hovered.epoch}</strong><span><i className="tooltip-dot" style={{background:colors.allow}}/>Allow {(hovered.values.allow*100).toFixed(1)}%</span><span><i className="tooltip-dot" style={{background:colors.block}}/>Block {(hovered.values.block*100).toFixed(1)}%</span><span><i className="tooltip-dot" style={{background:colors.human_review}}/>Human review {(hovered.values.human_review*100).toFixed(1)}%</span></div>}
    </div>
...
export function TrainingLive() {
  ...
  const liveTotal=Object.values(liveDistribution).reduce<number>((s,v)=>s+Number(v||0),0);
  ...
  return <div className="live-page">
    ...
    <section className="live-bottom-grid"><article><div className="panel-title">Selected models</div><div className="chips">{selected.length?selected.map(s=><span key={s}>{s}</span>):<span>—</span>}</div><div className="panel-title">Champion</div><strong>{best?.display_name??best?.name??"Not selected until comparison completes"}</strong></article><article><div className="panel-title">80-alert live holdout</div><div className="live-stats"><span>Expected<strong>{LIVE_EXPECTED}</strong></span><span>Considered<strong>{live?.alerts_considered??"—"}</strong></span><span>Processed<strong>{live?.alerts_processed??"—"}</strong></span></div><p>Human review <strong>{live?.human_review_routed??"—"}</strong></p><p>{live?.alerts_considered===LIVE_EXPECTED&&live?.alerts_processed===LIVE_EXPECTED?"All 80 real live alerts were processed.":"The live holdout is complete only when all 80 alerts have been considered and processed."}</p>{liveTotal>0&&<p>Actions: {Object.entries(liveDistribution).map(([action,count])=>`${action} ${count}`).join(" · ")}</p>}</article></section>
  </div>;
}
