...
# Silently reclaim only the project ports used by this local stack.
kill_port_listeners() {
  local port="$1"
  local pids=""
...
# Silently clear stale training processes so a restarted backend cannot return 409.
...
if ! "$PYTHON" -c 'import uvicorn' >/dev/null 2>&1; then
  die "uvicorn is not installed in the selected Python environment."
fi
if ! "$NPM" --version >/dev/null 2>&1; then
  die "npm is not usable."
fi

# Frontend dependency installation is silent and self-healing:
# npm ci -> npm install fallback -> show the log only if both fail.
